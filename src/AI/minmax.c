...
{
    if ( pos.first < 0 || pos.first > GRID_WIDTH || pos.second < 0 || pos.second > GRID_HEIGHT)
        return 0;
    ...
    return controled +
    compute_score(grid, ai, player,p1) +
...
/*int compute_score(int *grid, Pair ai, Pair player, Pair pos)
{
    return _compute_score(grid,GRID_WIDTH/2,GRID_HEIGHT/2,AI) - _compute_score(grid, GRID_WIDTH/2, GRID_HEIGHT/2, PLAYER);
}*/
...
Pair minmax_cpu(int *grid, Entity ai, Entity player, int deep)
{
    if (deep == 0)
    {
        ...
        Pair res = {.first=-1, .second=compute_score(grid, p_ai, p_ai, p_ai)};
        return res;
    }
