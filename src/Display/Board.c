...
int adjsut_grid_size(int *width, int *height) {
  (void)width;
  (void)height;
  struct winsize ws;
  ...
  int border_H_size = 3; // + header + 2 grids;
  int border_W_size = 4;
  ...
  if (new_height < minimal_grid_height + border_H_size ||
    ...
    printf("\e[%d;%dHNot enougth space!", new_height / 2, (new_width / 2) - 9);
    ...
    for (int i = 0; i < max_lines; i++) {
        ...
        if (i % 2 == 0) {
            if (i == 0) {
                ...
                addToString(buffer, "└", 3);
                for (int j = 0; j < max_cols; j++) {
                    (j != max_cols - 1 ? addToString(buffer, "────┴", 15) : NULL);
                    (j == max_cols - 1 ? addToString(buffer, "────┘", 15) : NULL);
                }
                addToString(buffer, "\n", 1);
            } else {
